...
from recgen_inference.recgen_modules.utils import elastic_utils
...
class BasicTrainer(Trainer):
    ...
    def load(self, load_dir, step=0):
        ...
        # Only load data_sampler if it exists (not None for IterableDataset like WebDataset)
    ...
    def save(self):
        ...
        # Only save data_sampler if it exists (not None for IterableDataset like WebDataset)
    ...
    def _load_checkpoint_file(self, path):
        """
        Load a checkpoint file, supporting both .pt (torch) and .safetensors formats.
        """
    ...
    def finetune_from(self, finetune_ckpt):
        ...
        for name, model in self.models.items():
            ...
            if name in finetune_ckpt:
                model_ckpt = self._load_checkpoint_file(finetune_ckpt[name])
                ...
                model_ckpts[name] = model_ckpt
                # Allow for partial loading of state dict as pose is not used in initial training
                # Later we can add a check to ensure that only pose weights are missing
                missing_keys, unexpected_keys = model.load_state_dict(model_ckpt, strict=False)
                if self.is_master and (missing_keys or unexpected_keys):
                    if missing_keys:
                        print(f'Missing keys (will use initialized values): {missing_keys}')
                    if unexpected_keys:
                ...
                if missing_keys:
                    ...
                    for key in missing_keys:
                        model_ckpt[key] = model_state_dict_after_load[key]
            else:
                if self.is_master:
    ...
    def run_step(self, data_list):
        ...
        for i, mb_data in enumerate(data_list):
            ...
            with nested_contexts(*sync_contexts), elastic_controller_context():
                ...
                ## check for NaN loss and skip this entire step
                if not torch.isfinite(l):
                    print(f'\033[93mWARNING: NaN/Inf loss detected, skipping this batch\033[0m')
                    nan_detected = True
                    break
                ## backward
                if self.fp16_mode == 'amp':
        ...
        if self.fp16_mode == 'amp':
            ...
            if not any(not p.grad.isfinite().all() for p in self.model_params):
                ...
                self.log_scale = max(self.log_scale - 1, -1000)
        else:
            prev_scale = 1.0
            if not any(not p.grad.isfinite().all() for p in self.model_params):
                self.optimizer.step()
        ...
        # Move additional metrics (bins, pose stats) to a separate log group
        if '_additional' in step_log['loss']:
            step_log['loss_additional'] = step_log['loss'].pop('_additional')
