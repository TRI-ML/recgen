...
def setup_dist(rank, local_rank, world_size, master_addr, master_port):
    ...
    # Read timeout from environment variable (in seconds), default 7200 seconds = 120 min.
    timeout_seconds = int(os.environ.get('NCCL_TIMEOUT', '7200'))
    timeout = timedelta(seconds=timeout_seconds)
    print(f'[Rank {rank}] Initializing NCCL with timeout={timeout_seconds}s ({timeout_seconds/60:.1f} min)')
